...
import { ArrowsUpDownIcon, UserCircleIcon, IdentificationIcon, PhoneIcon, TagIcon, PencilSquareIcon, CalendarDaysIcon, ClockIcon, BoltIcon, PaperAirplaneIcon, ArrowDownTrayIcon, TrashIcon, FunnelIcon } from '@heroicons/react/24/outline';
...
export default function Customers({ language }) {
  // State for expanded customer card (mobile)
  ...
  // Helper: handle long-press for mobile select
  ...
  // THIS IS THE KEY PART: Make sure this is correct and debug it
  const workflowName = localStorage.getItem("workflow");
  const [workflowDebug, setWorkflowDebug] = useState(workflowName);
  ...
  useEffect(() => {
    setWorkflowDebug(localStorage.getItem("workflow"));
    if (!workflowName) return;
  ...
  if (!workflowName) {
    ...
    return (
      <div className="p-10 bg-white dark:bg-gray-800 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <div className="mb-4">
          <label htmlFor="workflow-picker" className="block mb-2 font-semibold text-gray-900 dark:text-gray-100">
            {lang("selectWorkflow", language) || "Select a workflow:"}
          </label>
          <select
            id="workflow-picker"
            value={tempWorkflow}
            onChange={e => {
              setTempWorkflow(e.target.value);
              setInput(e.target.value);
            }}
            className="p-2 border rounded bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 mb-2 w-64"
          >
            <option value="">{lang("chooseWorkflow", language) || "Choose..."}</option>
            {options.map(opt => (
          ...
          <div className="mt-2">
            <input
              type="text"
              className="p-2 border rounded w-64 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              placeholder={lang("orEnterWorkflow", language) || "Or enter workflow key manually"}
              value={input}
              onChange={e => {
                setInput(e.target.value);
                setTempWorkflow(e.target.value);
        ...
        <button
          onClick={() => {
            if (input && input.trim()) {
              localStorage.setItem("workflow", input.trim());
        ...
        >
          {lang("continue", language) || "Continue"}
        </button>
      </div>
    );
  }
  if (customers.length === 0) {
    return (
      <div className="p-10 text-gray-700 bg-white dark:bg-gray-800">
        <div>🛑 <b>No customer data found for workflow:</b> <code>{workflowDebug}</code></div>
        <div>(Check your workflow key in localStorage, and ensure your DB has matching data!)</div>
      </div>
    );
  }
  return (
    ...
    <div className="flex flex-col h-[100vh] min-h-0 p-2" dir={localStorage.getItem("lang") === "ar" ? "rtl" : "ltr"}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 w-full">
        <h1
          className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white truncate"
          style={{ minWidth: 0 }}
        >
            ...
            <div ref={sortContainerRef} className="relative">
              ...
              {showSortMenu && (
                ...
                >
                  ...
                  >
                    {lang("firstSeenHeader", language) || "First Seen"} {sortConfig.key === "first_seen" ? (sortConfig.direction === "asc" ? "↑" : "↓") : ""}
                  </button>
                  <button
                    className={`block w-full text-left px-4 py-2 text-sm hover:bg-blue-50 dark:hover:bg-gray-800 ${
                  ...
                  >
                    {lang("lastSeenHeader", language) || "Last Seen"} {sortConfig.key === "last_seen" ? (sortConfig.direction === "asc" ? "↑" : "↓") : ""}
                  </button>
                </div>
              )}
            </div>
              ...
              {showFilterMenu && (
                ...
                >
                  <div className="mb-2 font-semibold text-gray-800 dark:text-gray-200 text-sm">
                    {lang("filterCustomers", language) || "Filter Customers"}
                  </div>
                  <div className="flex flex-col gap-3">
                    ...
                    <input
                      type="text"
                      className="w-full p-2 text-xs rounded border border-gray-300 dark:border-gray-600 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring focus:ring-blue-300 dark:focus:ring-blue-600"
                      placeholder={lang("firstSeenHeader", language) || "First Seen"}
                    ...
                    <input
                      type="text"
                      className="w-full p-2 text-xs rounded border border-gray-300 dark:border-gray-600 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring focus:ring-blue-300 dark:focus:ring-blue-600"
                      placeholder={lang("lastSeenHeader", language) || "Last Seen"}
      ...
      {/* iOS-style Floating Action Bar for Mobile Selection */}
      {isMobile && selectedCustomers.length > 0 && (
        <div
          className="fixed left-0 right-0 z-50 flex justify-center pointer-events-none"
          style={{
            bottom: 70, // ensure above navbar
            transition: "bottom 0.2s",
          }}
        >
          <div className="flex space-x-3 bg-white dark:bg-gray-900 rounded-2xl shadow-xl px-4 py-3 border border-gray-200 dark:border-gray-800 pointer-events-auto"
            style={{
              boxShadow: "0 8px 32px 0 rgba(60,60,60,0.12), 0 1.5px 5px 0 rgba(60,60,60,0.08)",
          ...
          >
            <button
              className="w-12 h-12 flex items-center justify-center rounded-full bg-blue-600 text-white hover:bg-blue-700 shadow transition"
            ...
            <button
              className="w-12 h-12 flex items-center justify-center rounded-full bg-green-500 text-white hover:bg-green-600 shadow transition"
              style={{ fontSize: 24, minWidth: 44, minHeight: 44 }}
            ...
            <button
              className="w-12 h-12 flex items-center justify-center rounded-full bg-yellow-400 text-white hover:bg-yellow-500 shadow transition"
              style={{ fontSize: 24, minWidth: 44, minHeight: 44 }}
            ...
            <button
              className="w-12 h-12 flex items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600 shadow transition"
              style={{ fontSize: 24, minWidth: 44, minHeight: 44 }}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {isMobile && mobileSelectMode && (
          <div className="fixed top-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-3 py-2 shadow-md"
            style={{
              minHeight: 56,
          ...
          >
            <button
              ...
              className="w-11 h-11 flex items-center justify-center rounded-full bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400 font-bold text-base shadow hover:bg-blue-200 dark:hover:bg-blue-800 transition"
              style={{ minWidth: 44, minHeight: 44 }}
              aria-label={selectedCustomers.length === paginatedCustomers.length ? "Clear All" : "Select All"}
            >
              {selectedCustomers.length === paginatedCustomers.length ? lang("clearAll", language) || "Clear All" : lang("Select All", language) || "Select All"}
            </button>
          ...
          <div className="space-y-4 mt-1 pb-32">
            {paginatedCustomers.map((cust, idx) => {
              ...
              return (
                <div
                  key={cust.id}
                  className="relative mx-2 bg-white dark:bg-gray-900 rounded-2xl shadow-xl"
                  style={{
                    userSelect: "none",
                    ...
                    minHeight: 80,
                    border: isSelected ? "2px solid #2563eb" : "1.5px solid #ececec",
                ...
                >
                  {/* Selection Checkbox overlays avatar, iOS-style */}
                  {mobileSelectMode && (
                    <div
                    ...
                    >
                      <label className="relative flex items-center justify-center">
                        <input
                          ...
                          className="w-6 h-6 rounded-full border-2 border-blue-500 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition"
                          style={{
                            minWidth: 18,
                            minHeight: 18,
                            width: 24,
                        ...
                        <span className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          {isSelected && (
                            <svg width="20" height="20" viewBox="0 0 20 20" className="text-white" fill="none">
                              <circle cx="10" cy="10" r="10" fill="#2563eb" />
                              <path d="M6 10.5l2 2 5-5" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                          )}
                        </span>
                      </label>
                  ...
                  {/* Removed edit (pencil) icon, top-right, from collapsed card view */}
                  <div className="flex flex-row items-center gap-4 px-5 pt-4 pb-2">
                    <div className="relative">
                      ...
                      />
                      {/* Checkbox overlays avatar, handled above */}
                    </div>
                  ...
                  <div className="flex justify-between px-5 pb-3 pt-1 text-xs text-gray-500 dark:text-gray-400">
                    <span>
                  ...
                  {(expandedCustomer === cust.phone || mobileSelectMode) && (
                    <div
                      className={`flex flex-row items-center justify-center gap-4 px-4 pb-3 pt-2 ${mobileSelectMode ? "opacity-50 pointer-events-none" : ""}`}
                      style={{
      ...
      {!isMobile && (
        <div className="sticky bottom-0 w-full bg-white dark:bg-gray-800 p-2 flex justify-center items-center">
